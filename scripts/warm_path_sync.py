...
def main() -> int:
    ...
    # pipeline.json - e.g. the graph has "Meta (Facebook)" but JobWatch's
    # ATS_BOARDS entry is plain "Meta", so scorer.py's exact-lowercase
    # lookup misses even though a warm-path row exists. company_aliases.json
    # already maps a short/alternate spelling to its canonical graph name for
    # this exact reason on the import side (graph_import.py) - reuse it here
    # to also sync a row under each alias spelling, pointing at the same
    # counts as its canonical company, so either spelling resolves.
